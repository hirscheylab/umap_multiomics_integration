...
save(out, file = "../results_single_cell/factorization.RData")
```
...
all_sc <- rbind(rgcca, mcia, mofa, intnmf, jive, tica, ubmi) %>% 
  as.data.frame() %>% 
...
ggsave(paste0(results_folder, "plot_overall.jpg"), width = 9, height = 9)
```

## UBMI Clusters

```{r}
ubmi_clusters <- data.frame(sample = rownames(ubmi), ubmi, clust = paste0("Cluster_", out$UBMI.clusters)) %>% 
...
ggsave(paste0(results_folder, "plot_UBMI_clusters.jpg"), width = 6, height = 5)
```
...
ubmi_clusters_subset <- ubmi_clusters[ubmi_clusters$clust %in% c("Cluster_2", "Cluster_3"),]
exp_subset <- exp[, colnames(exp) %in% rownames(ubmi_clusters_subset)]

poma_obj <- PomaCreateObject(metadata = ubmi_clusters_subset[, c("sample", "clust")],
...
poma_obj %>% 
  PomaVolcano(method = "limma",
              pval = "adjusted",
              pval_cutoff = 0.05,
              adjust = "fdr",
              log2fc_cutoff = 1.5)

ggsave(paste0(results_folder, "volcanoplot_clusters_2_3_UBMI.jpg"), width = 8)
```
